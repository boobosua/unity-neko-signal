...
namespace NekoSignal
{
    public static class SignalExtensions
    {
        ...
        /// <summary>Publishes a signal of the specified type with filters.</summary>
        public static void Publish<T>(this MonoBehaviour owner, T signal, params ISignalFilter[] filters) where T : ISignal
        {
            if (owner == null)
            {
                Log.Warn("[SignalExtensions] Cannot publish signal from null MonoBehaviour.");
                return;
            }
#if UNITY_EDITOR
        ...
        /// <summary>Disposes all signal receivers attached to the MonoBehaviour.</summary>
        public static void DisposeAllSignalReceivers(this MonoBehaviour owner)
        {
            if (owner == null)
            {
                Log.Warn("[SignalExtensions] Cannot dispose receivers for null MonoBehaviour.");
                return;
            }
        ...
        /// <summary>Wraps a signal into a fluent emit options pipeline for filtered publishing.</summary>
